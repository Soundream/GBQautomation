...
def main():
    ...
    try:
        # Updated path to queries.json
        with open('data_collection/sql/queries.json', 'r') as f:
            config = json.load(f)
    except Exception as e:
        ...
        for query_config in tqdm(folder_config.get('queries', []), 
            ...
            try:
                ...
                if not query and 'template' in query_config:
                    ...
                    # Update template path to include sql folder
                    template_path = f"data_collection/sql/{template_path}"
