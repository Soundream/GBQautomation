...
# API key
with Path('auth_credential/apikey.txt').open('r') as f:
...
def run_key_brand_1(start_date=None, end_date=None, yyyymm=None, output_folder=""):
    ...
    # List of companies and their domains (replace with the actual output from the grouped data)
    ...
    BASE_URL = "https://api.similarweb.com/v1/website/{}/dedup/deduplicated-audiences"

    # Set output path to the provided folder if given
    if output_folder:
        if yyyymm:
            output_path = Path(output_folder) / f"{yyyymm}_dedup_audience.csv" 
        else:
            output_path = Path(output_folder) / "dedup_audience.csv" 
    else:
        if yyyymm:
            output_path = Path(f"{yyyymm}_dedup_audience.csv")
        else:
            output_path = Path("dedup_audience.csv")

    # Iterate through each company and their domains
    for company, domains in companies_and_domains.items():
        for domain in domains:
            url = f"{BASE_URL}?api_key={API_KEY}&start_date={_start_date}&end_date={_end_date}&country=world&main_domain_only=false&format=json".format(domain)
            ...
            try:
                ...
                for entry in data.get("data", []):
                    ...
                    dedup_audience = int(entry["dedup_data"]["total_deduplicated_audience"])
